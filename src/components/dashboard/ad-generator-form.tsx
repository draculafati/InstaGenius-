        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                   <FormLabel className="text-white/80">Ad Prompt</FormLabel>
                  <FormControl>
                    <Textarea
                      placeholder="e.g., 'A summer sale for our new skincare line, focused on hydration.'"
                      className="resize-none bg-black/20 border-white/20 text-white placeholder:text-white/50 focus-visible:ring-primary/50"
                      rows={3}
                      {...field}
                    />
                  </FormControl>
             ...
             <FormField
              ...
              render={({ field }) => (
                <FormItem className="space-y-3">
                  <FormLabel className="text-white/80">Media Type</FormLabel>
                   <FormControl>
            ...
            <Button 
              ...
              disabled={isLoading} 
              size="lg"
              className="w-full text-lg bg-white text-gray-900 hover:bg-gray-200 h-12"
            >
              {isLoading ? (
                <>
      ...
      {isLoading && (
         <div className="grid gap-8 md:grid-cols-2">
            ...
            <Card className="bg-white/10 backdrop-blur-lg border border-white/20 text-white">
              ...
              <CardContent>
                 <Skeleton className="aspect-video w-full bg-white/20" />
                 <p className="text-xs text-white/60 mt-2 text-center">Video generation may take up to a minute.</p>
              </CardContent>
            </Card>
            <Card className="md:col-span-2 bg-white/10 backdrop-blur-lg border border-white/20 text-white">
              <CardHeader>
            ...
            <div className="grid gap-8 md:grid-cols-2">
                {generatedContent.imageDataUri && (
                    <Card className="bg-white/10 backdrop-blur-lg border border-white/20 text-white">
                        ...
                        <CardContent>
                            <Image src={generatedContent.imageDataUri} alt="Generated Ad Image" width={1920} height={1080} className="rounded-lg border border-white/20" />
                        </CardContent>
                    </Card>
                )}
                 {generatedContent.videoDataUri && (
                    <Card className="bg-white/10 backdrop-blur-lg border border-white/20 text-white">
                        ...
                        <CardContent>
                            <video src={generatedContent.videoDataUri} controls className="w-full rounded-lg border border-white/20" />
                        </CardContent>
                    </Card>
                 )}
            </div>
             <Card className="bg-white/10 backdrop-blur-lg border border-white/20 text-white">
                ...
                <CardContent className="space-y-4">
                    <Textarea value={generatedContent.caption} readOnly rows={5} className="bg-black/20 border-white/20 text-white" />
                    <div className="flex flex-wrap gap-2">
                        {generatedContent.hashtags.map((tag) => (
                            <Badge key={tag} variant="secondary" className="bg-white/20 text-white hover:bg-white/30">{tag}</Badge>
                        ))}
                    </div>
                </CardContent>
            </Card>
            <div className="flex justify-end">
                <Button onClick={handleSaveAd} className="bg-white text-gray-900 hover:bg-gray-200">
